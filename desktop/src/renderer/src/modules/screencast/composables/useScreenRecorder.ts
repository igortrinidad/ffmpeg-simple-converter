...
const CAMERA_BUBBLE_RATIO = 0.15
...
function drawFrame(
  ...
  if (cameraVideo) {
    const radius = Math.min(canvas.width, canvas.height) * CAMERA_BUBBLE_RATIO
    const cx = CAMERA_BUBBLE_MARGIN + radius
    const cy = canvas.height - CAMERA_BUBBLE_MARGIN - radius
    ...
    ctx.clip()
    ctx.drawImage(cameraVideo, sx, sy, size, size, cx - radius, cy - radius, radius * 2, radius * 2)
    ctx.restore()
...
async function startRecording(options: StartRecordingOptions): Promise<void> {
  ...
  const loop = (): void => {
    drawFrame(ctx, canvas, screenVideo, cameraVideo)
